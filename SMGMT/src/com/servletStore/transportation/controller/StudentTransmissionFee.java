	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(request.getParameter("route_name")!=null)
		{
			try
			{
				...
				System.out.print("Tname"+request.getParameter("route_name"));
				while(rs.next())
				{					
		...
		else if(request.getParameter("route_name1")!=null&&request.getParameter("dest_name")!=null)
		{
			try
			{
				pstmt=connection.prepareStatement("SELECT * FROM destination_master,route_master where destination_master.route_id=route_master.id and route_master.route_name=? and destination_master.destination=?");
